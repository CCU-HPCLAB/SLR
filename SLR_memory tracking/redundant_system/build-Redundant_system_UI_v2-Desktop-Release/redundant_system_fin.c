...
int count2=0;
void *funcall( void *ptr, int *chdpid,void *programpath)//, int *chdflag) 
{
   	int fd = *(int *)ptr;
	int cd = *(int *)chdpid;
	char flag[256]="1";
	...
	if ((pp = popen("sudo ./top.sh ", "r")) == NULL) {  
		printf("popen() error!\n");  
		exit(1);  
	}  
	...
	if(strcmp(buf,"danger")){
		...
		//identify primary system keep alive or not.
		//if ( Flag ==2 &&(strcmp(buf,"ok")))
			if(count2==0){
			system("pkill -9 dmtcp");
			system("~/build-Redundant_system_UI_v2-Desktop-Release/mv_restart.sh");
			// execute external program
			system("/dmtcp/dmtcp_restart_script_12085f9b-40000*.sh --restartdir /dmtcp/ --no-strict-checking -h localhost");
			count2++;
			}
...
void *writefile( )
{
	...
	while(1){
		...
		if ((pp = popen("sudo ./mnt_gluster.sh ", "r")) == NULL) {  
			printf("popen() error!\n");  
			exit(1);  
		}  
		...
		{
			//printf("mnt_gluster is NULL\n");
		}
		else{
			sleep(8);
			system("~/build-Redundant_system_UI_v2-Desktop-Release/mv.sh");
			break;
		}
	...
	while(1){
		...
		if ( reconnection_count != 0 ){
			if(reconnection_count==1)
			{
				...
				while(1){
						chdpid=getpid();
						printf("funcall frist process chdpid:\n");			
						funcall((void *)&newSocket,&chdpid,programpath);
		...
		if(reconnection_count==0){
				...
				while(1){
					...
					chdpid=getpid();
					printf("writefile process chdpid: %d\n",chdpid);			
					writefile();
					waitpid(-getpid(), &stat, 0);
					exit(EXIT_SUCCESS);
					}
